...
2. Иначе берёт тот, что сработал в прошлый раз (он лежит в профиле)
3. Иначе идёт напрямую
4. Если напрямую не вышло — сама подбирает рабочий прокси и повторяет

Найденный адрес запоминается, поэтому следующие запуски идут сразу через него.
