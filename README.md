...
2. Снова запустите `run.bat` и выберите нужный источник (например, **Пункт 1** для сбора всех базовых списков или **Пункт 3** для получения списка по китайским ссылкам).
   - Скрипт начнет по очереди переходить по ссылкам и нажимать "Add to My Assets" / принимать лицензионные соглашения.
