...
| Способ | Как |
|---|---|
| **Прокси только для Telegram** (рекомендую) | Пункт **T** в `run.bat` или `--tg-proxy socks5://127.0.0.1:1080`. Unity продолжает ходить напрямую — быстрее и не вызывает вопросов у Unity |
| VPN на весь компьютер | Через него пойдёт и Unity тоже |
